...
def main(config):
    ...
    # create data folders
    save_folder = os.path.join('../data/output', config['mode'])
    ...
    for epoch_i in range(epoch_num+1):
        for _ in range(0, sample_num, batch_size):
            ...
            _, batch_loss = sess.run((train_op, loss), feed_dict={input_fea: batch_fea, input_label: batch_label})
            print('batch loss: %f' % batch_loss)

        acc = sess.run(pred_accuracy, feed_dict={input_fea: eval_data, input_label: eval_labels})
        # print(acc)
