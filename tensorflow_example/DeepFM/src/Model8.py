...
feature_size = 436
embedding_size = 10
dnn_layers = [400, 200, 64]
cross_layers = 3
cross_dnn_layers = [100]
concat_input_size = 174  # embedding_size + deep_layers[-1] + crossDeep_layers[-1]
layers_activation = tf.nn.relu
batch_size = 256
learning_rate = 0.001
optimizer_type = "adam"
...
def initialize_weights():
    ...
    # fm embedding
    weights["fm_embeddings"] = tf.Variable(
        tf.random_normal([feature_size, embedding_size], 0.0, 0.01),
        name="fm_embeddings")  # N * K

    # deep layers
    dnn0_input_size = feature_size  # N
    glorot = np.sqrt(2.0 / (dnn0_input_size + dnn_layers[0]))

    weights["dnn_w_0"] = tf.Variable(
        np.random.normal(loc=0, scale=glorot, size=(dnn0_input_size, dnn_layers[0])),
        dtype=np.float32, name="dnn_w_0")
    weights["dnn_b_0"] = tf.Variable(np.random.normal(loc=0, scale=glorot, size=(1, dnn_layers[0])),
                                     dtype=np.float32, name="dnn_b_0")  # 1 * layers[0]
    for i in range(1, len(dnn_layers)):
        glorot = np.sqrt(2.0 / (dnn_layers[i - 1] + dnn_layers[i]))
        weights["dnn_w_%d" % i] = tf.Variable(
            np.random.normal(loc=0, scale=glorot, size=(dnn_layers[i - 1], dnn_layers[i])),
            dtype=np.float32, name="dnn_w_%d" % i)  # layers[i-1] * layers[i]
        weights["dnn_b_%d" % i] = tf.Variable(
            np.random.normal(loc=0, scale=glorot, size=(1, dnn_layers[i])),
            dtype=np.float32, name="dnn_b_%d" % i)  # 1 * layer[i]

    # cross layer
    weights["cross_w"] = tf.get_variable(name='cross_w', shape=[cross_layers, feature_size],
                                         initializer=tf.glorot_normal_initializer())
    weights["cross_b"] = tf.get_variable(name='cross_b', shape=[cross_layers, feature_size],
                                         initializer=tf.glorot_normal_initializer())

    # cross-dnn layers
    dnn0_input_size = feature_size  # N
    glorot = np.sqrt(2.0 / (dnn0_input_size + cross_dnn_layers[0]))
    weights["cross_dnn_w_0"] = tf.Variable(
        np.random.normal(loc=0, scale=glorot, size=(dnn0_input_size, cross_dnn_layers[0])),
        dtype=np.float32, name="cross_dnn_w_0")
    weights["cross_dnn_b_0"] = tf.Variable(np.random.normal(loc=0, scale=glorot, size=(1, cross_dnn_layers[0])),
                                           dtype=np.float32, name="cross_dnn_b_0")  # 1 * crossDeep_layers[0]

    for i in range(1, len(cross_dnn_layers)):
        glorot = np.sqrt(2.0 / (cross_dnn_layers[i - 1] + cross_dnn_layers[i]))
        weights["cross_dnn_w_%d" % i] = tf.Variable(
            np.random.normal(loc=0, scale=glorot, size=(cross_dnn_layers[i - 1], cross_dnn_layers[i])),
            dtype=np.float32, name="cross_dnn_w_%d" % i)  # layers[i-1] * layers[i]
        weights["cross_dnn_b_%d" % i] = tf.Variable(
            np.random.normal(loc=0, scale=glorot, size=(1, cross_dnn_layers[i])),
            dtype=np.float32, name="cross_dnn_b_%d" % i)  # 1 * layer[i]

    # final concat projection layer
    glorot = np.sqrt(2.0 / (concat_input_size + 1))
    weights["concat_w1"] = tf.Variable(
        np.random.normal(loc=0, scale=glorot, size=(concat_input_size, 1)),
        dtype=np.float32, name="concat_w1")  # layers[i-1]*layers[i]
    weights["concat_w2"] = tf.Variable(
        np.random.normal(loc=0, scale=glorot, size=(concat_input_size, 1)),
        dtype=np.float32, name="concat_w2")  # layers[i-1]*layers[i]
    weights["concat_b1"] = tf.Variable(tf.constant(0.01), dtype=np.float32, name="concat_b1")
    weights["concat_b2"] = tf.Variable(tf.constant(0.01), dtype=np.float32, name="concat_b2")
...
def train(config):
    ...
    embeddings = tf.multiply(weights["fm_embeddings"], reshaped_input, name="fm_embeddings")  # None * N * K
    ...
    # --- cross component ---
    # 1.cross
    cross_layer_0 = inputs  # None * N
    y_cross = cross_layer_0  # None * N
    for layer in range(cross_layers):
        cross_w = tf.reshape(weights["cross_w"][layer], shape=[-1, 1])
        xlw = tf.matmul(y_cross, cross_w)  # None * 1
        y_cross = cross_layer_0 * xlw + y_cross + weights["cross_b"][layer]
    # 2.dnn
    for i in range(0, len(cross_dnn_layers)):
        y_cross = tf.add(tf.matmul(y_cross, weights["cross_dnn_w_%d" % i]), weights["cross_dnn_b_%d" % i])
        if batch_norm_Flag:
            y_cross = batch_norm_layer(y_cross, train_phase=train_phase, scope_bn="cross_b_%d" % i)
        y_cross = layers_activation(y_cross)

    # --- dnn component ---
    y_dnn = inputs
    for i in range(0, len(dnn_layers)):
        y_dnn = tf.add(tf.matmul(y_dnn, weights["dnn_w_%d" % i]), weights["dnn_b_%d" % i])  # None * layer[i] * 1
        if batch_norm_Flag:
            y_dnn = batch_norm_layer(y_dnn, train_phase=train_phase, scope_bn="bn_%d" % i)  # None * layer[i] * 1
        y_dnn = layers_activation(y_dnn)

    # --- ncf ---
    version = int(config.version)
    if version == 3:
        concat_input = tf.concat([y_second_order, y_cross], axis=1)
    else:
        concat_input = tf.concat([y_second_order, y_dnn, y_cross], axis=1)
    logits1 = tf.add(tf.matmul(concat_input, weights["concat_w1"]), weights["concat_b1"])
    logits2 = tf.add(tf.matmul(concat_input, weights["concat_w2"]), weights["concat_b2"])
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for i in xrange(1, epoch_number + 1):
            ...
            if i % 500 == 0:
                ...
                layer = [k[0] for k in train_batch["label"]]
                auc = roc_auc_score(layer, s)
                # print(zip(s, l))
...
def test(config):
    ...
    with tf.Session(graph=g, config=tf_config) as sess:
        ...
        while True:
            ...
            s3 = [0.6 * i + 0.4 * j for (i, j) in zip(s1, s2)]
...
def printParameter():
    ...
    print("deep_layers      ", dnn_layers)
    print("cross_layers     ", cross_layers)
    print("crossDeep_layers ", cross_dnn_layers)
    print("concat_input_size", concat_input_size)
...
def init_parameters(config):
    version = int(config.version)
    global feature_size, embedding_size, dnn_layers, cross_layers, cross_dnn_layers, concat_input_size
    global layers_activation, batch_size
    global learning_rate, optimizer_type, batch_norm_Flag, batch_norm_decay, verbose
    global random_seed, l2_reg, epoch_number

    if version == 1:
        cross_dnn_layers = []
        concat_input_size = 510  # embedding_size+deep_layers[-1]+feature_size
    if version == 2:
        cross_dnn_layers = [100]
        concat_input_size = 174  # embedding_size+deep_layers[-1]+crossDeep_layers[-1]
    if version == 3:
        cross_dnn_layers = [400, 200, 64]
        concat_input_size = 74  # embedding_size+crossDeep_layers[-1]
