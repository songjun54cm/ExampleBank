...
def train(config):
    ...
    if int(config.version) == 2:
        play_score = tf.add(tf.matmul(concat_input, weights["concat_projection2"]), weights["concat_bias2"],
                            name="score2")
    else:
        play_score = tf.nn.sigmoid(logits2, name="score2")

    if int(config.version) in [3, 4]:
    ...
    if int(config.version) in [5]:
        opt = tf.train.GradientDescentOptimizer(
            tf.train.exponential_decay(0.000001, global_step, 100, 0.96, staircase=True))
    else:
