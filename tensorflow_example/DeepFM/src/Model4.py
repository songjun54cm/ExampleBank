...
def train(config):
    ...
    logits1 = tf.add(tf.matmul(concat_input, weights["concat_projection1"]), weights["concat_bias1"], name="logits1")
    logits2 = tf.add(tf.matmul(concat_input, weights["concat_projection2"]), weights["concat_bias2"], name="logits2")

    # --- loss ---
    sub_score = tf.nn.sigmoid(logits1, name="score1")
    loss1 = tf.losses.log_loss(labels, sub_score)

    if int(config.version) == 4:
        play_score = tf.constant(logits2, name="score2")
    else:
        play_score = tf.nn.sigmoid(logits2, name="score2")
    if int(config.version) == 5:
        loss2 = tf.losses.log_loss(labels, play_score)
    else:
...
def train3(config):
    ...
    sub_score = tf.nn.sigmoid(logits1, name="score1")
    play_score = tf.nn.sigmoid(logits2, name="score2")
    score = tf.multiply(sub_score, play_score, name="score")
    loss1 = tf.losses.log_loss(labels, score)
    loss2 = tf.reduce_mean(tf.square(playPercent - play_score))
    loss = tf.add(loss1, loss2)
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for i in xrange(1, epoch_number + 1):
            ...
            if i % 500 == 0:
                # train
                res1 = sess.run([score, accuracy, loss1, loss2, loss],
                                feed_dict={inputs: train_batch["features"],
                                           labels: train_batch["label"],
                                           playPercent: train_batch["playPercent"],
                                           train_phase: False})
                ...
                # dev
                dev_batch = sess.run(next_dev_batch)
                res2 = sess.run([score, accuracy, loss1, loss2, loss],
                                feed_dict={inputs: dev_batch["features"],
                                           labels: dev_batch["label"],
                                           playPercent: train_batch["playPercent"],
                                           train_phase: False})
...
def test(config):
    ...
    with tf.Session(graph=g, config=tf_config) as sess:
        ...
        while True:
            ...
            count += 1
            batch_score = sess.run([sub_scores, play_scores], feed_dict={"input:0": test_batch["features"],
        ...
        print("Finished, time consumed: %s hours %s minutes %s seconds." % (
            str(int((endTime - startTime) / 3600)), str(int(((endTime - startTime) % 3600) / 60)),
            str((endTime - startTime) % 60)))
        all_auc1 = roc_auc_score(all_label, all_score1)
        all_auc3 = roc_auc_score(all_label, all_score3)
        print("All Test  AUC1 = %f, AUC3 = %f, "
              "time_1 = %f, time_2 = %f, time_3 = %f, "
              "pct_1 = %f, pct_2 = %f, pct_3 = %f" % (
                  all_auc1, all_auc3,
...
if __name__ == '__main__':
    ...
    if mode == "train" and version <= 2:
        train(config)
    elif mode == "train" and version == 3:
        train3(config)
    elif mode == "train" and version == 6:
        train6(config)
    elif mode == "test":
        test(config)
    else:
